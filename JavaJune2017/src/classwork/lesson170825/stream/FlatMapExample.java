...
//		
////		Stream<Stream<String>> map = stringArrays.map(sa -> Arrays.stream(sa));
//		Stream<String> streamOfLetters = stringArrays.flatMap(sa -> Arrays.stream(sa));
//		
//		List<String> distinctLetters = streamOfLetters.distinct().collect(Collectors.toList());
//		System.out.println(distinctLetters);
		

		System.out.println(Arrays.stream(words).map(s -> s.split("")).flatMap(Arrays::stream).distinct().collect(Collectors.toList()));
